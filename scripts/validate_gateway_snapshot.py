"""Validate the dedicated gateway-repository snapshot without leaking host paths."""
...
from app.gateway_repo.snapshot import validate_gateway_snapshot


def validate_and_build(snapshot_path: Path, *, require_read_only: bool) -> dict[str, object]:
    snapshot = validate_gateway_snapshot(
        snapshot_path,
        require_read_only=require_read_only,
    )
    content = GatewayRepositoryContent(snapshot)
    content.repository_context(max_chars=1_000)
    return {
